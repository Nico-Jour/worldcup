...
echo "$($PSQL "SELECT t.name FROM games g JOIN teams t ON g.winner_id = t.team_id WHERE g.round = 'Eighth-Final' AND year = 2014 UNION SELECT t.name FROM games g JOIN teams t ON g.opponent_id = t.team_id WHERE g.round = 'Eighth-Final' AND year=2014 GROUP BY name ORDER BY name;")"
